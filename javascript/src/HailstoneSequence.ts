import { SequenceState, stoppingTimeTerminus } from './utilities.js';
import { collatzFunction } from './function.js';
...
export class HailstoneSequence {
  ...
  constructor(initialValue: bigint, P: bigint, a: bigint, b: bigint, maxTotalStoppingTime: number, totalStoppingTime: boolean) {
    ...
    if (initialValue === 0n) {
      ...
      // Otherwise, hail!
      const minMaxTotalStoppingTime: number = Math.max(maxTotalStoppingTime, 1);
 ...
 * Thrown if either P or a are 0.
 */
export function hailstoneSequence({ initialValue, P = 2n, a = 3n, b = 1n, maxTotalStoppingTime = 1000, totalStoppingTime = true }: CollatzHailstoneParameters): HailstoneSequence {
