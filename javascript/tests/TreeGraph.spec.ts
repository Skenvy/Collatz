...
function assertExpectedTree(actualParams: Collatz.CollatzTreeGraphParameters, expectedRoot: Collatz.TreeGraphNode): void {
  ...
  assert.deepEqual(actual.root, expectedRoot);
}
...
describe('treeGraph', () => {
  ...
  it('should return the assertion error', () => {
    assert.throws(() => { Collatz.treeGraph({ initialValue: 1n, maxOrbitDistance: 1, P: 0n, a: 2n, b: 3n }); }, Collatz.FailedSaneParameterCheck, Collatz.SaneParameterErrMsg.SANE_PARAMS_P);
    assert.throws(() => { Collatz.treeGraph({ initialValue: 1n, maxOrbitDistance: 1, P: 0n, a: 0n, b: 3n }); }, Collatz.FailedSaneParameterCheck, Collatz.SaneParameterErrMsg.SANE_PARAMS_P);
    assert.throws(() => { Collatz.treeGraph({ initialValue: 1n, maxOrbitDistance: 1, P: 1n, a: 0n, b: 3n }); }, Collatz.FailedSaneParameterCheck, Collatz.SaneParameterErrMsg.SANE_PARAMS_A);
  });
});
