...
To version our packages we're using, rather than just always installing the latest, we can use a package version manager. R has two that appear to be common; [packrat](https://CRAN.R-project.org/package=packrat) and [renv](https://CRAN.R-project.org/package=renv). [Packrat](https://rstudio.github.io/packrat/) describes how it works [here](https://rstudio.github.io/packrat/walkthrough.html) and [renv](https://rstudio.github.io/renv/) has additional details [here](https://rstudio.github.io/renv/articles/renv.html); renv even has a [packrat vs renv](https://cran.r-project.org/web/packages/renv/vignettes/packrat.html) page. Although packrat looks like it has wider adoption from existing for longer, renv comes with the guarantee that it'll be at least of `Hadley Wickham` quality, so we'll try out renv.
### renv
...
## renv soapbox
Renv is not easy to use. It might be easy _once it's setup_, but it's a bit on the nose for renv to compare themselves to packrat in terms of ease of use when renv has been nothing but foot-guns so far in the attempt to get it setup to begin with.

All the renv docs and videos by the RStudio team discussing renv highlight how "easy" it is to use. It _should_ be easy. It'd be nice if it _was_ easy. But so far my experience with renv has been anything _but_ easy. From the beginning, it didn't want to initialise properly, and I had to play around with it to figure out how to actually initialise my project. Once I'd gotten it safely initialised, it took a while to figure out how and why snapshots taken in different places weren't behaving as I would've expected them to. All the confusion at each stage stems from not having looked further into those specific functions.

Which makes this complaint more lukewarm than some of my other complaints, because most of the information I feel like I'm missing when something doesn't happen as expected does actually exist in the docs, but buried further in them; hidden deeper in the docs than I initially anticipated having to look, for a tool frequently described as "easy to use" and compares itself as a much simpler to use tool than packrat. Perhaps part of my frustration comes from the manner in which I am trying to use it is apparently not the expected use case, and renv is more precise rather than a blunt tool. The other half of the frustration stems from it trying to be too smart in the way that it self-determines what dependencies it manages. There's some control to say how it should discover what to require, and for someone trying to set up an environment to run something in, sure, that sounds very easy for them.

But it _hasn't_ been easy trying to set up renv for the use case of producing a consistent **development environment**. By the time I've been trying to set it up for several days, and get around to reading the [FAQ](https://rstudio.github.io/renv/articles/faq.html), we can see this is somewhat by design;
...
Although it would be great if renv itself offered a way to manually add dependencies (without adding a workaround `_dependencies.R` like suggested [here](https://rstudio.github.io/renv/reference/dependencies.html)) to see why it isn't, we can check what dependencies it thinks it needs with `Rscript -e 'renv::dependencies()'`, which will inform us what top level dependencies it can pick up from parsing our code. The output from it is a list of files it's read, and the subsequent packages it's identified. The dependencies loaded during a snapshot can be altered by the snapshot type (in `./renv/settings.json`, there's a `"snapshot.type"`). Use `Rscript -e 'renv::settings$snapshot.type("<type>")'`, `implicit` being the default type. The others are `all`, or `explicit`. The `Rscript -e 'renv::dependencies()'` will be unaffected by this, the output would be what the `implicit` snapshot would expect.

The FAQ advises against hand rolling the renv.lock file, although mentions it's a possibility, and the fix it recommends for development dependencies not being installed is to use `renv::install()`, which does not use the lockfile, but will install dependencies listed in the `./DESCRIPTION`. But the whole need for this stems from a version change to one of the "suggests" packages. So I think the eventuality we'll just have to accept is adding the hacky workaround of a file just for renv to parse to gobble up the other dependencies. It feels somewhat ironic that, in the search for how to add versioned package management to our R build, the answer offered in a lot of places online is renv. But the fix for renv's inadequacies for our use case of a consistent development environment is much more aligned with what I would've expected a list of depended packages to be, closer to a pip requirements. So we'll need a file that just invokes each of the packages we want to add in renv, and then use renv to build the lockfile from that list of requirements, which feels like hijacking the source of truth. It could also be done with [profiles](https://rstudio.github.io/renv/articles/profiles.html) in the `./DESCRIPTION`.
